...
**协议一旦上线就很难修改**——因为你要同时兼容新老版本的客户端。所以协议设计必须"向前看"，预留扩展空间，考虑未来的业务需求。
...
网络游戏中的协议可以按传输方向、可靠性和使用场景分为八种基本类型。理解这八种类型，才能为不同的业务场景选择正确的协议。
...
- **登录操作**：必须可靠，延迟可以稍高。用请求-响应协议。
- **位置同步**：必须低延迟，偶尔丢包可以接受。用 UDP 实时同步。
- **系统公告**：必须可靠（不能丢），但延迟不敏感。用服务端推送。
...
好的命名规范让协议自解释——看到消息 ID 就知道它是哪个模块的什么操作。推荐的命名格式是：`模块_动作_方向`。
...
分片重组的一个常见陷阱是：**超时清理**。如果某个分片一直不到（网络丢包），重组器会一直占用内存。必须设置超时时间，超时后丢弃不完整的分片组。
...
速率限制是防止客户端发送过多消息导致服务端过载的关键机制。推荐使用**令牌桶算法**：每个客户端有一个令牌桶，桶里有一定数量的令牌，每秒补充一定数量。发送消息消耗令牌，令牌不足时拒绝消息。
