...
这种峰谷特征意味着：服务器必须按峰值配置，但大部分时间都在"空转"。这就是为什么弹性伸缩如此重要——在低峰期释放资源，节省成本。
...
> **陷阱**：压测最常见的错误是"测试环境与生产环境差异太大"——测试环境用了更好的硬件，或者没有模拟真实的网络延迟。压测结果看起来很好，上线后却扛不住。务必在与生产环境一致的条件下压测。
...
**日志量估算**（10万在线）：每个玩家每分钟约50条日志，每条500字节，每日约3.6TB。必须做好日志分层存储：热数据7天SSD，温数据30天HDD，冷数据90天+对象存储。

> **陷阱**：日志最常见的问题是"日志风暴"——某个异常循环大量打印日志，导致磁盘写满、系统崩溃。务必设置日志速率限制，单个进程的日志输出不能超过阈值。
...
任何发布都必须有回滚方案。回滚不是"可选的"，而是"必须的"。回滚方案需要在发布前就准备好，而不是出了问题才临时想办法。
