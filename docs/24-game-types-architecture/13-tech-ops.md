...
> 以下内容整合自《压测容量》专题，讨论容量规划、扩缩容与迁移的工程实践。
...
- 预测性扩容：基于历史数据提前扩容（周五晚8点自动扩容到峰值）
- 反应式扩容：基于实时指标触发（CPU > 70% 时自动增加实例）
- 定时缩容：低峰期自动缩减（凌晨2点-6点缩减到最小实例）
- 预留实例：基础负载用预留实例（折扣）
...
**常见误区**：把扩容理解成加机器；把多地域理解成多部署；不去解决状态和路由问题。
