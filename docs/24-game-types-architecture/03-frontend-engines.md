...
> 以下内容整合自《平台生态SDK》和《引擎语言工具》专题。

### 平台与渠道生态

平台不是中性的运行环境。它会带来审核规则、包体限制、接口差异、支付通道和商业化要求，这些都可能改变客户端、服务端和发布管线。

**平台约束会反向塑造实现方式**，不能等到上线前才补。

**常见做法**：

- 用适配层隔离 SDK、把平台能力抽象成项目内接口
- 围绕版本、权限和失败补偿设计兜底链路
...
**常见误区**：让平台 SDK 直接侵入核心逻辑；忽略渠道差异导致测试和回归成本失控。

### 发布、版本与商业模式

**版本管理**：

- 客户端版本、资源版本、协议版本需要统一管理
- 版本兼容矩阵：哪些客户端版本能连哪些服务端
- 灰度发布和回滚能力
...
- 表现需求：画面品质、特效复杂度
- 平台覆盖：iOS/Android/Web/PC/主机
- 人员供给：团队熟悉度、招聘难度
- 商业授权：Unity/Unreal 的授权模式
- 调试工具：性能分析、崩溃收集
- 资产管线：美术工作流、资源管理
...
- **Go**：高并发、低延迟、部署简单，适合游戏服务器
- **C++**：极致性能，适合高性能场景
- **Java**：生态丰富，适合复杂业务逻辑
- **Python**：快速原型，适合工具链和运营脚本
- **Rust**：内存安全+高性能，适合核心组件

**客户端语言选择**：

- **C#（Unity）**：最广泛的移动端游戏开发语言
- **C++（UE5）**：高性能游戏开发
- **TypeScript（Cocos/Laya）**：Web 和小游戏开发
...
- 手游：Unity（人才最多）、Cocos Creator（小游戏+手游一体化）
- PC/主机 3A：Unreal Engine 5（画面顶级）
- 小游戏：Cocos Creator（生态最好）、LayaAir
- 独立游戏：Godot（开源免费）

**常见误区**：把演示效果和最终生产效率混为一谈；忽略组织对工具链的消化能力。
